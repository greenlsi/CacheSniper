...
/*Access N+N-1 elements in the cache*/
int probe_reprobe(long int *pos_data, long int *pos_data1)
{
    volatile unsigned int time;
#if CACHE_SET_SIZE == 12
    asm __volatile__(
    ...
    lfence();
    unsigned long *begin1 = (unsigned long *)(pos_data1); //First read
    int cont = 0;
    lfence();
    do
    {
        begin1 = (unsigned long *)(*begin1);
        cont++;
    } while (cont < CACHE_SET_SIZE);
    time = (int)(timestamp() - t1);
#endif
...
/*Flush desired contents*/
void reset_all_ages(long int address_array[CACHE_SET_SIZE])
{
    int i = 0;
    long int *test_add = (long int *)address_array[i];
    i = probe_one_set(test_add);
//    printf("p %i \n",i);
    while (i > 50*CACHE_SET_SIZE)
    {
        i = probe_one_set(test_add);
//	printf("reset %i \n",i);
    }
...
int probe_candidate(int number_elements, long int array_set[], long int *dir_candidate, int time_main_memory)
{
    ...
    lfence();
    //int t = access_timed(dir_candidate);
    //printf("%i ",t);
    //return t > time_main_memory;
